...
function checktime(month, day, year, hour) {
  ...
  if (month > 0 && month < 13) {
    if (day < 0 || day > mm[month])
      throw `there are not ${day} days in this month `;
...
module.exports = {
  async createAppointment(userId, parkId, activityId, year, month, day, hour) {
    if (!userId || !parkId || !activityId || !year || !month || !day || !hour)
      throw "please provide all inputs for appointment";
    ...
    if (
      Number(hour) < Number(park.openTime.slice(0, 2)) ||
      Number(hour) >= Number(park.closeTime.slice(0, 2))
    )
    ...
    let appointment = await appointmentCollection
      .find({
    ...
    if (appointment.length != 0) {
      if (appointment.find((x) => x.user.includes(ObjectId(userId)))) {
        throw "you can't submit the same appointment";
      } else {
        let matchedappointment = appointment.find(
          (element) => element.user.length < Number(activity.limit)
        );
        if (matchedappointment) {
          matchedappointment.user.push(ObjectId(userId));
          if (matchedappointment.user.length == Number(activity.limit))
            matchedappointment.status = true;
          ...
          if (
            appointment.length ==
            Number(activity.maxPeople) / Number(activity.limit)
          )
            throw "The number of activity's appointment is max, you can't make an appointment";
          let status = false;
          ...
          const insertInfo = await appointmentCollection.insertOne(
            newAppointment
          );
  ...
  async getAppointmentByemail(email) {
    if (!email) throw "must provid email for appointment";
  ...
  async getAppointmentById(appointmentId) {
    if (!appointmentId) throw "must provide appointmentId for appointment";
    if (arguments.length != 1) throw "the number of parameter is wrong";
    const appointmentCollection = await appointments();
    let appointment = await appointmentCollection.findOne({
  ...
  async cancelAppointmentById(appointmentId) {
    ...
    if (typeof appointmentId === "undefined")
      throw "appointmentId is undefined!";
    if (arguments.length != 1) throw "the number of parameter is wrong";
    if (!ObjectId.isValid(appointmentId) && typeof appointmentId !== "string")
      throw "appointmentId is not a string or objectKey!";
  ...
  async getAllAppointmentByActivityID(activityId) {
    if (!activityId) throw "must provide activityId for appointment";
    if (arguments.length != 1) throw "the number of parameter is wrong";
    const appointmentCollection = await appointments();
    let appointment = await appointmentCollection
      .find({

  ...
  if (!req.session.user) {
    ...
    try {
      ...
      for(let i=0;i<Appointments.length;i++){
        ...
        res.status(200).render('function/Appointment_myAppointment', { data: appointments })
    } catch (e) {
...
router.post('/cancelAppointment', async (req, res) => {
  if (!req.session.user) {
    ...
    try {
      var body = req.body;
      if (!body.appointmentId) throw "Please provide all the input for cancelAppointmentById!"
...
router.post('/newAppointment', async (req, res) => {
  if (!req.session.user) {
    ...
    try {
      var body = req.body;
      if (!body.park || !body.activity || !body.year || !body.month || !body.day || !body.hour) throw "Please provide all the input for create newAppointment!";
